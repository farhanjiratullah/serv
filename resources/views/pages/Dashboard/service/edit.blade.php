                                ...
                                <div class="px-4 py-5 sm:p-6">
                                    <div class="grid grid-cols-6 gap-6">
                                        ...
                                        <div class="col-span-6">
                                            ...
                                            @foreach ($service->AdvantageServices as $advantageService)
                                                <input placeholder="Keunggulan Service" type="text"
                                                    name="{{ 'advantage-services[' . $advantageService->id . ']' }}"
                                                    id="{{ 'advantage-services[' . $advantageService->id . ']' }}"
                                                    autocomplete="{{ 'advantage-services[' . $advantageService->id . ']' }}"
                                                    class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm"
                                                    value="{{ $advantageService->advantage }}"
                                                    {{ $loop->first ? 'required' : '' }}>
                                            @endforeach

                                            <div id="newServicesRow"></div>

                                            @error('advantage-services[]')
                                                <p class="text-red-500 mb-3 text-sm">{{ $message }}</p>
                                            @enderror
                                        ...
                                        <div class="col-span-6 sm:col-span-3">
                                            <select id="revision_limit" name="revision_limit" autocomplete="revision_limit"
                                                ...
                                                <option value="2" {{ $service->revision_limit == 2 ? 'selected' : '' }}>2
                                                    Revisi</option>
                                                <option value="5" {{ $service->revision_limit == 5 ? 'selected' : '' }}>5
                                                    Revisi</option>
                                                <option value="7" {{ $service->revision_limit == 7 ? 'selected' : '' }}>7
                                                    Revisi</option>
                                                <option value="10"
                                                    {{ $service->revision_limit == 10 ? 'selected' : '' }}>10
                                                    Revisi</option>
                                                <option value="12"
                                                    {{ $service->revision_limit == 12 ? 'selected' : '' }}>12
                                                    Revisi</option>
                                            </select>
                                        ...
                                        <div class="col-span-6">
                                            ...
                                            <div class="grid grid-cols lg:grid-cols-3 md:grid-cols-2 gap-4">
                                                @foreach ($service->Thumbnails as $thumbnail)
                                                    <div class="mb-2">
                                                        <img src="{{ Storage::url($thumbnail->thumbnail) }}"
                                                            alt="Thumbnail" class="inline object-cover w-20 h-20 rounded"
                                                            for="choose">

                                                        <input placeholder="Thumbnail" type="file"
                                                            name="{{ 'thumbnails[' . $thumbnail->id . ']' }}"
                                                            id="{{ 'thumbnails[' . $thumbnail->id . ']' }}"
                                                            autocomplete="{{ 'thumbnails[' . $thumbnail->id . ']' }}"
                                        ...
                                        <div class="col-span-6">
                                            ...
                                            @foreach ($service->AdvantageUsers as $advantageUser)
                                                <input placeholder="Keunggulan Kamu" type="text"
                                                    name="{{ 'advantage-users[' . $advantageUser->id . ']' }}"
                                                    id="{{ 'advantage-users[' . $advantageUser->id . ']' }}"
                                                    autocomplete="{{ 'advantage-users[' . $advantageUser->id . ']' }}"
                                                    class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm"
                                                    value="{{ $advantageUser->advantage }}"
                                                    {{ $loop->first ? 'required' : '' }}>
                                            @endforeach

                                            <div id="newAdvantagesRow"></div>

                                            @error('advantage-users[]')
                                                <p class="text-red-500 mb-3 text-sm">{{ $message }}</p>
                                            @enderror
                                        ...
                                        <div class="col-span-6">
                                            ...
                                            @foreach ($service->Taglines as $tagline)
                                                <input placeholder="Tagline" type="text"
                                                    name="{{ 'taglines[' . $tagline->id . ']' }}"
                                                    id="{{ 'taglines[' . $tagline->id . ']' }}"
                                                    autocomplete="{{ 'taglines[' . $tagline->id . ']' }}"
                                                    class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm"
