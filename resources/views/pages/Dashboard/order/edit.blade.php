                                        ...
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                ...
                                                @if ($order->OrderBuyer->DetailUser->photo)
                                                    <img src="{{ Storage::url($order->OrderBuyer->DetailUser->photo) }}"
                                                        alt="{{ $order->OrderBuyer->name }}"
                                            ...
                                            <div>
                                                <p class="font-medium text-black">
                                                    {{ $order->OrderBuyer->name }}</p>
                                                <p class="text-sm text-gray-400">
                                                    {{ $order->OrderBuyer->DetailUser->contact_number }}</p>
                                            </div>
                                        </div>
                                    </td>
                            ...
                            <div class="flex p-8 border border-gray-200 rounded-lg bg-serv-upload-bg h-128">
                                <div class="m-auto text-center">
                                    <img src="{{ asset('assets/images/services-file-icon.svg') }}" alt=""
                                        class="w-20 mx-auto">
                                    <h2 class="mt-8 mb-1 text-2xl font-semibold text-gray-700">
                                        {{-- @dd($order->file->getClientOriginalExtension()) --}}
                                        @if ($order->file)
                                            {{ substr($order->file, -44) }}
                                        @else
                                    ...
                                    <div class="relative mt-0 md:mt-6">
                                        ...
                                        @else
                                            <input type="file" name="file" hidden id="choose" accept=".zip" required>
                                            <label for="choose"
                                                class="px-4 py-2 mt-2 text-left text-gray-700 rounded-xl bg-serv-hr">
                                                Choose File
                                            </label>
                                    ...
                                    <div class="grid grid-cols-6 gap-6">
                                        <div class="col-span-6">
                                            ...
                                            <textarea placeholder="Enter your biography here.." type="text" name="note"
                                                ...
                                                rows="4" required
                                                {{ $order->file ? 'disabled' : '' }}>{{ $order->note ?? '' }}</textarea>
