        ...
        <div class="grid gap-5 md:grid-cols-12">
            <main class="p-4 lg:col-span-8 md:col-span-12">
                ...
                <div class="p-3 my-4 bg-gray-100 rounded-lg image-gallery" x-data="gallery()">
                    <img :src="featured" alt="{{ $service->title }}" class="rounded-lg cursor-pointer w-100" data-lity>
                    <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                        <div class="flex mt-2 flex-nowrap">
                            ...
                            @foreach ($service->Thumbnails as $thumbnail)
                                <img :class="{'border-4 border-serv-button': active === {{ $thumbnail->id }} }"
                                    @click="changeThumbnail('{{ Storage::url($thumbnail->thumbnail) }}', {{ $thumbnail->id }})"
                                    src="{{ Storage::url($thumbnail->thumbnail) }}" alt="{{ $service->title }}"
                    ...
                    <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }"
                        ...
                        <div x-show.transition.duration.500ms="tab === 'description'" class="leading-8 text-md">
                            ...
                            <ul class="mb-4 list-check">
                                ...
                                @foreach ($service->AdvantageUsers as $advantageUser)
                                    <li class="pl-10 my-2">{{ $advantageUser->advantage }}</li>
                                @endforeach
                            ...
                            @if ($service->User->DetailUser->contact_number)
                                <p class="mb-4 font-medium">
                                    Contact me to get started!
                                    <br>
                                    This my contact number: {{ $service->User->DetailUser->contact_number }}
                                </p>
                            @endif
                        ...
                        <div x-show.transition.duration.500ms="tab === 'seller'" class="leading-8 text-md">
                                ...
                                <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        @if ($service->User->DetailUser->photo)
                                            <img src="{{ Storage::url($service->User->DetailUser->photo) }}"
                                                alt="{{ $service->User->name }}"
                                                class="w-20 h-20 object-cover rounded-full" loading="lazy">
                                        @else
                                            <svg class="w-20 h-20 rounded-full text-gray-300 object-cover"
                            ...
                            <div class="mt-4 mb-8 content-description">
                                <p>
                                    {{ $service->User->DetailUser->biography }}
                                </p>
                            </div>
                            <h3 class="my-4 text-lg font-semibold">My Experiences</h3>
                            <ul class="mb-8 list-check">

                                @forelse ($service->User->DetailUser->ExperienceUsers as $experienceUser)
                                    <li class="pl-10 my-2">{{ $experienceUser->experience }}</li>
                                @empty
                                    <p>I don't have any experiences.</p>
                                @endforelse

                            </ul>
                            <h3 class="my-4 text-lg font-semibold">Taglines</h3>
                            <div class="mb-8 skills">

                                @forelse ($service->Taglines as $tagline)
                                    <span
                                        class="inline-block px-3 py-1 mr-2 rounded bg-serv-services-bg">{{ $tagline->tagline }}</span>
                                @empty
                                    <p>There's no tagline in this service.</p>
                                @endforelse

                            </div>
                            <hr class="border-serv-services-bg">
                            <p class="my-4 text-lg text-gray-400">
                                Joined Since {{ $service->User->created_at->format('d F Y') }}
                            </p>
                        </div>
            ...
            <aside class="p-4 lg:col-span-4 md:col-span-12 md:pt-0">
                <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                    ...
                    <div class="flex items-start px-4 pt-6">
                        ...
                        @if ($service->User->DetailUser->photo)
                            <img src="{{ Storage::url($service->User->DetailUser->photo) }}"
                                alt="{{ $service->User->name }}" class="object-cover w-16 h-16 mr-4 rounded-full"
                                loading="lazy">
                        @else
                    ...
                    <div class="px-4 pt-4 pb-2 features-list">
                        <ul class="mb-4 text-sm list-check">
                            ...
                            @foreach ($service->AdvantageServices as $advantageService)
                                <li class="pl-10 my-4">{{ $advantageService->advantage }}</li>
                            @endforeach
                    ...
                    <div class="px-4 pb-4 booking">
                        @auth
                            <form action="{{ route('booking.landing', $service->id) }}" method="post">
                                @csrf

                                <button type="submit"
                                    class="block w-full px-12 py-4 my-2 text-lg font-semibold text-center text-white bg-serv-button rounded-xl">
                                    Booking Now
                                </button>
                            </form>
                        @endauth
...
@push('after-script')
    <script>
        function gallery() {
            return {
                featured: '{{ Storage::url($service->Thumbnails->first()->thumbnail) }}',
                active: '{{ Storage::url($service->Thumbnails->first()->id) }}',
