        ...
        <div class="m-auto text-center h-auto">
            <svg class="mb-6 w-full" width="490" height="350" viewBox="0 0 490 350" fill="none"
                ...
                <defs>
                    ...
                    <!-- Gambar kiri -->
                    @if ($order->OrderBuyer->DetailUser->photo)
                        <image id="image0" width="500" height="500"
                            xlink:href="{{ Storage::url($order->OrderBuyer->DetailUser->photo) }}"
                            alt="{{ $order->OrderBuyer->name }}" loading="lazy" />
                    @else
                        <svg class="w-500 h-500 rounded-full text-gray-300 object-cover" fill="currentColor"
                            viewBox="0 0 24 24" id="image0">
                            <path
                    ...
                    @if ($order->OrderFreelancer->DetailUser->photo)
                        <image id="image1" width="500" height="500"
                            xlink:href="{{ Storage::url($order->OrderFreelancer->DetailUser->photo) }}"
                            alt="{{ $order->OrderFreelancer->name }}" loading="lazy" />
                    @else
                        <svg class="w-500 h-500 rounded-full text-gray-300 object-cover" fill="currentColor"
                            viewBox="0 0 24 24" id="image1">
                            <path
                                d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                        </svg>
                    @endif
            ...
            @if ($order->OrderFreelancer->DetailUser->contact_number[0] == 0)
                @php
                    $firstDigit = $order->OrderFreelancer->DetailUser->contact_number[0];
                    $contact_number = $firstDigit == 0 ? substr($order->OrderFreelancer->DetailUser->contact_number, 1) : '';
                @endphp

                <a href="http://wa.me/62{{ $contact_number }}"
                    class="bg-serv-button text-white text-md font-medium py-4 w-80 my-2 rounded-2xl text-center inline-block">
                    Continue to Chat
                </a>
            @elseif($order->OrderFreelancer->DetailUser->contact_number)
                <a href="http://wa.me/{{ $order->OrderFreelancer->DetailUser->contact_number ?? '' }}"
                    class="bg-serv-button text-white text-md font-medium py-4 w-80 my-2 rounded-2xl text-center inline-block">
                    Continue to Chat
                </a>
            @else
                <p>
                    Sorry, this freelancer not set his contact number yet, Please wait him to accept your request
                </p>
